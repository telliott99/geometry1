...
By the forward version of the isosceles triangle theorem, the entire $A = B$, so it follows that all four angles dotted blue and magenta are equal.

Then $\triangle ABD \cong \triangle BAF$ by ASA.

As a result, $AF = BD$ and $AD = BF$.  

Furthermore, since the original triangle is isosceles and $AD = BF$, the smaller triangle $\triangle CDF$ is also isosceles, by subtraction.  Alternatively, $C$ is shared, and the long sides are equal so $\triangle CDB \cong \triangle CFA$ by AAS.
...
In $\triangle ABC$ draw the bisector of $C$ to join side $AB$, dividing it into lengths $x$ and $y$.
...
Start from the point where the bisector meets the side opposite $C$, cutting $c$ into $x$ and $y$.  Drop the perpendiculars to the sides $a$ and $b$.  
...
$\triangle ABD \sim \triangle AEC$, since $A$ is shared.
...
\subsection*{problem from A. Bogomolny}

Here is a nice problem I found on A. Bogomolny's site.  It is from an Olympiad, but I'd bet it is probably much older than that.

\begin{center} \includegraphics [scale=0.3] {rtbisect1.png} \end{center}

In right $\triangle ABC$, bisect the acute angles.  Find the points $P$ and $R$ on the opposite sides and drop perpendiculars to the hypotenuse at $M$ and $N$.  Connect $CM$ and $CN$.

Find the measure of $\angle MCN$.

\emph{Proof}.

$\triangle ACR$ and $\triangle ANR$ are both right triangles containing an acute angle which is one-half of $A$.  They share $AR$, so they are congruent.  Thus $CR = NR$ and $\angle NCR = \angle RNC$.

Similarly, $PC = PM$ and $\angle PCM = \angle CMP$.  

We have two kites.

The perpendicular from $C$ to the hypotenuse (not drawn) divides $\angle MCN$ into two parts, 
one part is equal to $\angle CNR$ and one is equal to $\angle CMP$ (by alternate interior angles).

But these two angles are also equal to those lying on the other side of $CN$ and $CM$ because they are in isosceles triangles.

It follows that $\angle MCN$ is one-half of a right angle.

$\square$

\begin{center} \includegraphics [scale=0.3] {rtbisect2.png} \end{center}

In addition, $X$ lies on the intersection of the bisectors, so it is the incenter of $\triangle ABC$.

Because of the kites $PX$ is the perpendicular bisector of $CM$ and $RX$ is the same for $CN$.

Thus, $X$ is also the circumcenter for $\triangle CMN$.

It follows that $\angle MXN$ is the central angle for $\angle MCN$ and is thus a right angle.
