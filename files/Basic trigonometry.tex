...
Here is another simple proof.

\begin{center} \includegraphics [scale=0.3] {sine_law.png} \end{center}

In the circumcircle for $\triangle ABC$, draw the diameter through $C$ with length $2R$, and label the other endpoint as $J$.  Since they lie on the same arc, $\angle A = \angle J$.  But then clearly

\[ \sin A = \sin J = \frac{a}{2R} \]
\[ \frac{a}{\sin A} = 2R \]

and this can be done for any of the three angles in $\triangle ABC$.

\subsection*{problem}

Use the same idea to extend this proof to the case where $\angle A$ is obtuse.  Hint:  $\sin A = \sin (180 - A)$.
...
\subsection*{length of altitude}

The lengths of the two parts of the altitude (vertex to orthocenter, orthocenter to foot) may be computed as follows.

Recall the law of sines from earlier in this chapter:
\[ \frac{a}{\sin A} = \frac{b}{\sin B} = \frac{c}{\sin C} \]
\begin{center} \includegraphics [scale=0.3] {sines.png} \end{center}

In the diagram above, we see that with $\theta$ as one-half of the central angle for an arc of the circle, $\sin \theta = d/R$, where $d$ is one-half the chord length. Rearrange to give
\[ 2R = \frac{2d}{\sin \theta} \]
and substitute into the law of sines
\[ \frac{a}{\sin A} = \frac{b}{\sin B} = \frac{c}{\sin C} = 2R \]

\begin{center} \includegraphics [scale=0.25] {two_altitudes_b.png} \end{center}

In the figure above, the angle opposite $C$ at $H$ is supplementary to $\angle C$ because the other two angles in the quadrilateral $ADHE$ are right.  (Alternatively, form a cyclic quadrilateral).  The sine of $\angle DHE$ is equal to that of $C$ and so is the sine of $\angle AHB$.

Since $\triangle ABE$ is right, $\sin ABH$ is equal to $\cos A$.

In $\triangle AHB$ apply the law of sines:
\[ \frac{c}{\sin C} = \frac{AH}{\cos A} \]

but $c/\sin C = 2R$ thus

\[ AH = 2R |\cos A| \]

for obtuse $A$, $\cos A$ may be negative, hence we take the absolute value.

\begin{center} \includegraphics [scale=0.25] {two_altitudes_b.png} \end{center}

Similarly $\sin DBH = \cos C$ so 

\[ \frac{HD}{BH} = \sin DBH = \cos C \]
\[ HD = BH \cos C \]

We use the law of sines:

\[ \frac{BH}{\sin BAH} = \frac{BH}{\cos B} = \frac{c}{\sin C} = 2R \]
\[ BH = 2R \cos B \]
...
Since they are supplementary, $\angle B + \angle C = 180 - \angle A$, and using the law of sines:
...
This is called the sum of angles formula for sine.  Show that the first part works for obtuse $\angle B$.
