...
In the circumcircle for $\triangle ABC$, draw the diameter through $C$ with length $2R$, and label the other endpoint as $J$.  Since they lie on the same arc, $A = J$.  But then clearly
...
Use the same idea to extend this proof to the case where $A$ is obtuse.  Hint:  $\sin A = \sin (180 - A)$.
...
There are several other common representations of the six functions.  We'll see more when we get to analytic geometry.  Here is a common one where the tangent to the circle, perpendicular to the ray with $\theta$ is equal to $\tan \theta$.  See if you can work this out by using the complementary angle to $\theta$.
...
In the figure above, the angle opposite $C$ at $H$ is supplementary to $C$ because the other two angles in the quadrilateral $ADHE$ are right.  (Alternatively, form a cyclic quadrilateral).  The sine of $\angle DHE$ is equal to that of $C$ and so is the sine of $\angle AHB$.
