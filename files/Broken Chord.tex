...
$A$ is bisected since the two parts are subtended by $BD = HD$.
...
$G = A$, as inscribed angles on the same arc.
...
We have $SAS$ in the small right triangles so $\triangle DBE \cong \triangle DZE$, which means $DB = DZ$.  We have $GD = AD$ as before and $A = G$.
...
Just to be clear which sides and angles we are talking about:  the angles we know are $A$ and $G$.  We follow around the sides $AD = GD$ and then $DB = DZ$.  The angle which may be either acute or obtuse is the one opposite the first side $AD$ or $GD$.

We have, for example, $\triangle ADZ$ with an obtuse angle at $Z$, then $\triangle ADB$ with an acute angle at $B$, and finally $\triangle GDB$ with an obtuse angle at $B$.  $\triangle ADZ \cong \triangle GDB$ because they both contain those obtuse angles.
...
(re-phrasing):  $GD = AD$ by the property of the median, $BD$ is shared, and $G = A$, so we have SSA.
