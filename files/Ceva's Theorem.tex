...
For a physical object with constant density, the point $P$ would be the \emph{center of mass}.
...
Then $A$ is part of two right triangles, and by similar triangles we have that
...
Here's a problem from Acheson (due to Viviani) that looks challenging, but yields easily to the right perspective.
...
And here is a PWOW (proof without words):

\begin{center} \includegraphics [scale=0.4] {Viviani_pwow.png}  \end{center}
...
We have a theorem that says the bisectors of any angle in a triangle divide the opposite side in the same proportions as the adjacent sides.  Use this fact and the converse of Ceva's theorem to show that incenter $I$ exists, i.e. that the bisectors are concurrent at $I$.
