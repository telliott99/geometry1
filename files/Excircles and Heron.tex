...
\emph{Proof}.  The proof just uses what we established above.  Let the triangle be $\triangle ABC$ with side $a$ opposite vertex $A$, etc., as usual, and let $AI$ be the bisector of $A$, $BI$ the bisector of $B$, and so on.  Drop perpendiculars from $I$ to the sides at $X$, $Y$ and $Z$.
...
We will often refer to the half-angles formed by bisection (not labeled here), named to remind us of the parent angle:   $\alpha + \alpha = A$ and $\alpha = \angle IAZ$.
...
\begin{center} \includegraphics [scale=0.35] {excircle_crop3.png} \end{center}
...
\begin{center} \includegraphics [scale=0.35] {excircle_crop1.png} \end{center}

We can see that this will be correct, since $BD = BF$ as tangents from $B$ and $CE = CF$ as tangents from $C$, so the bisectors of $\angle DBF$ and $\angle ECF$ will go through the center of the circle.
...
This result can be interpreted in the following way.  The area of $\triangle ABC$ is the whole area of  quadrilateral $ADI_aE$ minus the area of polygon $ECBDI_a$.  The first area is easily seen to be equal to $R_as$.

We can show that the latter area is equal to $R_a \cdot a$.  The area of $I_aFBD$ is $R_a \cdot BF$ ($BF$ is what we called $p$ above).  Similarly, the area of $I_aFCE$ is equal to $R_a \cdot CF$ so the total is equal to $R_a$ times the whole side $a$.

\subsection*{4}

\begin{center} \includegraphics [scale=0.35] {excircle_crop2.png} \end{center}
...
Another way to see this is that the external angle at $B$ is equal to $A$ plus $C$, so $\angle I_a BD$ is one-half that.  Since $\triangle I_a DB$ is right, it follows that $\angle B I_a D$ is one-half $B$.  
...
By the converse of Thales' theorem, $B$ and $I$ lie on the same circle, with diameter $CL$.  It follows that $BICL$ is a cyclic quadrilateral and therefore $L$ is supplementary to $\angle BIC$.  

But the latter is supplementary to $\beta + \gamma$ in $\triangle BIC$ (using our standard notation for the half-angles at $B$ and $C$).  Hence $L = \beta + \gamma$.

This is much easier to see if we draw the circle on diameter $CL$:  the arc corresponding to $L$ is divided between $\beta$ and $\gamma$.
...
It follows that $L = \angle AIF$, and then the two right triangles are similar:  $\triangle CBL \sim \triangle AIF$.
