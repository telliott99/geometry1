...
This theorem is used extensively in geometry.  It relates the side lengths of any triangle to the cosine of an angle.  For example, if the side opposite $A$ is $a$, then $a^2 = b^2 + c^2 - 2bc \cos A$.

Rewritten appropriately, it can give any side in terms of the other two.

Consider $\triangle ABC$ in two versions.  In one (left panel), $A$ is acute, and in the other (right panel) $A$ is obtuse.  Note that the supplementary $\angle BAD$ on the right is equal to $A$ from the acute case.  In particular, it has the same cosine, $x/h$.

Let the sides opposite be $a,b,c$, as usual. 
\begin{center} \includegraphics [scale=0.40] {law_of_cosines5b.png} \end{center}
...
$x/c$ is the cosine of $A$ so $x = c \cos A$ so then finally
\[ a^2 = b^2 + c^2 - 2bc \cos A \]

\begin{center} \includegraphics [scale=0.40] {law_of_cosines5b.png} \end{center}

For the obtuse case the initial arithmetic has a change of sign.  ($a$ is still the side opposite $A$, but it is obviously bigger now).
\[ a^2 = h^2 + (b+x)^2 \]
\[ c^2 = h^2 + x^2 \]
...
Thus, when we think about $A$ as the angle at vertex $A$ \emph{inside} the triangle
\begin{center} \includegraphics [scale=0.40] {law_of_cosines5b.png} \end{center}
...
where $A$ is $\angle BAC$ for the acute case and $\angle B'AC$ for the obtuse case.

This is the law of cosines.  We compute the length of one side $a$ in terms of the two other sides $b$ and $c$ and the angle between them, $A$.  This proof is identical to Euclid Book II.12 and II.13, except for the last step.
...
Here is derivation based on the crossed chord theorem.  Consider a general triangle with sides $a,b$ and $c$ and angle $\theta$ between $a$ and $c$

Draw a right triangle with one angle equal to $\theta$ on the diameter of a circle scaled to have radius $a$.  

%\begin{center} \includegraphics [scale=0.35] {law_of_cosines2.png} \end{center}
\begin{center} \includegraphics [scale=0.3] {pwow11_label.png} \end{center}

Mark off length $c$ on the long base of the right triangle, then draw a second diameter that includes side $b$.  The entire base is $2a \cos \theta$ since $a$ is the radius.  The lengths of the other parts are easy to compute. We have
\[ (a + b)(a - b) = c (2a \cos \theta - c) \]
\[ a^2 - b^2 = 2ac \cos \theta - c^2 \]
