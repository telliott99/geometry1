...
We also need to prove the theorem for an alternative setup.  Even though the line $XYZ$ in the figure below does not go through $\triangle ABC$, it is still a transversal.  Imagine that we start at the vertex $B$, run along $BY$ to the black line, run back along $CY$ to the top, and so on.

\begin{center} \includegraphics [scale=0.35] {menelaus11.png} \end{center}

By drawing the figure with the transversal horizontal, we see the similarity to the first version.

\[ \frac{h_1}{h_3} = \frac{BX}{AX} \ \ \ \ \ \ \ \ \frac{h_3}{h_2} = \frac{AY}{CY}  \ \ \ \ \ \ \ \ \frac{h_2}{h_1} = \frac{CZ}{BZ} \]

Multiply the left-hand sides to obtain $1$, thus
\[  \frac{BX}{AX}  \cdot  \frac{AY}{CY}  \cdot \frac{CZ}{BZ} \]

We can remember this as, start from a vertex, run to the line, then to the next vertex.  Also, take the first length we run into, and put it on top of the ratio.  It is a bit confusing because if we think of ratios between sides and parts of sides we have part/side $\cdot$ side/part $\cdot$ part/side.

We extend this result below.

\subsection*{alternative definition}
There is a more sophisticated version of Menelaus's Theorem which views the path around a triangle as consisting of \emph{directed} (that is, \emph{signed}) line segments.
