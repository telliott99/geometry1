...
Above is a graphic from wikipedia that shows where we're going in the second proof.  We will form two sets of similar triangles and use our knowledge about corresponding ratios.
...
In the proof below, we denote the area of polygons such as $ABCD$ using parentheses, as $(ABCD)$.  We will also drop the factor of $\frac{1}{2}$ by computing \emph{twice} the area of each figure.
...
This diagram below shows a simpler way of seeing the same result.
\begin{center} \includegraphics [scale=0.5] {Ptolemy13.png} \end{center}
\[ h_1 + h_2 = AC \sin \theta \]

We're on to something.  Before we go on, notice that $\angle AMB$ is external to $\triangle AMD$ so it is equal to the sum of angles $\angle ADM$ (red dot) and $\angle DAM$ (black dot).

Now, the really great idea.  Move the point $D$ to $D'$ such that $AD' = CD$ and $AD = CD'$.
\begin{center} \includegraphics [scale=0.16] {pt2b.png} \end{center}

When we do this, note two things.  First, $\angle CAD = \angle ACD'$ (black dots), because the arcs they intercept are equal, $AD' = CD$.  The whole $\angle BCD'$ (red dot plus black dot) is equal to $\angle AMB$ from above.
...
but since this is a cyclic quadrilateral it is also equal to
\[  2(ABD') = AB \cdot AD'  \cdot \sin \angle BCD' \]

and
\[ 2(CBD') = BC \cdot CD' \cdot \sin \angle BCD' \]
so the total area is 
\[ (AB \cdot AD'  + BC \cdot CD' )  \sin \angle BCD' \]

since $AD = CD'$ and $CD = AD'$
\[ 2 ABCD) = 2(ABCD') = (AB \cdot CD + BC \cdot AD) \sin \angle BCD' \]

We are almost there.  Referring back to first part, $\angle BCD' = \angle AMB$.  Thus, their sines are equal.
\begin{center} \includegraphics [scale=0.16] {pt2b.png} \end{center}

Bringing in the main result from the first part and canceling the sine of equal angles:
\[ AB \cdot CD + BC \cdot AD = AC \cdot BD \]
